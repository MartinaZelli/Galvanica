...
public class GestisciAggiunteMetodiComuni {
	public static List<StoricoTotaleSingoloDto> getStoricoTotaleSingoloDtoList(List<StoricoDettaglio> storicoDettaglioList) {
	...
	private static StoricoTotaleSingoloDto storicoTotaleSingoloDtoBuilder(StoricoDettaglio dettaglio) {
		...
		if (alimentazione == null) {
			alimentazione = Alimentazione.builder().scatti(null).idAlimentazione(null).build();
		}
		UnitaDiMisura unitaDiMisura =
			convertiUnitaMisuraPerDto(dettaglio.getQuantita(), dettaglio.getUnitaDiMisura().isSonoVolume());
	...
	public static List<StoricoTotaleGroupDto> getStoricoTotaleGroupDtoList(List<StoricoDettaglio> storicoDettaglioList) {
		...
		for (StoricoDettaglio storicoDettaglio : storicoDettaglioList) {
			...
			if (mappa.containsKey(key)) {
				Double quantitaProdotto = mappa.get(key).getQuantitaProdotto() + storicoDettaglio.getQuantita();
				mappa.get(key).setQuantitaProdotto(quantitaProdotto);

				mappa.get(key).getIdStoricoDettaglioList().add(storicoDettaglio.getIdStoricoDettaglio());
			}
			if (!mappa.containsKey(key)) {
		...
		for (StoricoTotaleGroupDto storico : mappa.values()) {
			...
			UnitaDiMisura unitaDiMisura = convertiUnitaMisuraPerDto((int) Math.round(storico.getQuantitaProdotto()),
				storico.getUnitaDiMisura().isSonoVolume());
			Double quantita =
				convertiQuantitaGenerico(storico.getQuantitaProdotto(), storico.getUnitaDiMisura(), unitaDiMisura);
	...
	private static StoricoTotaleGroupDto storicoTotaleGroupDtoBuilder(StoricoDettaglio dettaglio) {
	...
	public static List<StoricoTotaleGroupDto> queryTransformerPerGroup(List<Map<String, Object>> listaQuery) {
		List<StoricoTotaleGroupDto> storicoTotaleGroupDtoList = new ArrayList<>();
		for (Map<String, Object> query : listaQuery) {
			StoricoTotaleGroupDto storicoTotaleGroupDto = StoricoTotaleGroupDto.builder().build();
			...
			if (query.containsKey("quantitaProdotto")) {
				storicoTotaleGroupDto.setQuantitaProdotto((Double) query.get("quantitaProdotto"));
			}
			if (query.containsKey("unitaDiMisura")) {
				storicoTotaleGroupDto.setUnitaDiMisura((UnitaDiMisura) query.get("unitaDiMisura"));
			}
			...
			if (query.containsKey("idAlimentazione")) {
				storicoTotaleGroupDto.setIdAlimentazione((Long) query.get("idAlimentazione"));
			}
			if (query.containsKey("idStoricoDettaglioList")) {
				...
				storicoTotaleGroupDto.setIdStoricoDettaglioList(idStoricoDettaglioList);
			}
			if (storicoTotaleGroupDto.getUnitaDiMisura() != null &&
				storicoTotaleGroupDto.getQuantitaProdotto() != null) {
				UnitaDiMisura unitaDiMisura =
					convertiUnitaMisuraPerDto((int) Math.round(storicoTotaleGroupDto.getQuantitaProdotto()),
						storicoTotaleGroupDto.getUnitaDiMisura().isSonoVolume());
				Double quantita = convertiQuantitaGenerico(storicoTotaleGroupDto.getQuantitaProdotto(),
