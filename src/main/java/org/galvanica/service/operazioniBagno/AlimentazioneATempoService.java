...
public class AlimentazioneATempoService {
	...
	// todo: attenzione, quando si confermano le aggiunte storico precedenti vanno
	// approvate in ordine crescente di data, mai al contrario o non tornano gli
	// scatti totali e parziali
	// todo: attenzione! in questo momentoo viene ricercato su StoricoGenerale tutte
	// gli storici (sia con il parametro scatti che con il parametro tempo), da
	// implementare controllo!
	public AlimentazioneATempoService(StoricoGeneraleRepository storicoGeneraleRepository,
		StoricoDettaglioRepository storicoDettaglioRepository,
		AlimentazioneRepository alimentazioneRepository, BagnoService bagnoService) {
	...
	public void calcolaAlimentazione(Long idBagno, LocalDate dataControllo) {
		...
		date.forEach(data -> {
			List<Alimentazione>
				alimentazioneList =
				alimentazioneRepository.findByTempo(idBagno, data.getDayOfWeek().name());
	...
	private void creaStorico(Alimentazione alimentazione, LocalDate dataControllo) {

		StoricoGenerale
			storicoGenerale =
			storicoGeneraleRepository.save(StoricoGenerale.builder().alimentazione(alimentazione)
				.bagno(alimentazione.getBagno()).tipologiaAggiunta(TipologiaAggiunta.TEMPO)
				.dataControlloTempo(dataControllo).build());
		for (DettaglioAlimentazione dettaglio : alimentazione.getDettaglioAlimentazioneList()) {
			...
			storicoDettaglioRepository.save(
				StoricoDettaglio.builder().prodotto(dettaglio.getProdotto())
					.storicoGenerale(storicoGenerale).quantita(dettaglio.getQuantitaProdotto())
					.unitaDiMisura(dettaglio.getUnitaDiMisura()).build());
		}
	}

	public List<StoricoTotaleSingoloDto> calcolaAggiunteStoricoPerIdBagnoDaDataAData(Long idBagno,
		...
		List<StoricoDettaglio>
			...
			storicoGeneraleListDaEseguire.stream().filter(
					storicoGenerale -> storicoGenerale.getStoricoDettaglioList() != null &&
						!storicoGenerale.getStoricoDettaglioList().isEmpty()).filter(
					s -> s.getDataControlloTempo().isAfter(dataFrom.minusDays(1)) &&
	...
	private StoricoTotaleSingoloDto buildPerSingolo(StoricoDettaglio dettaglio) {
		...
		return StoricoTotaleSingoloDto.builder()
			...
			.quantitaProdotto(quantita).unitaDiMisura(unitaDiMisura)
			...
			.dataControlloTempo(dettaglio.getStoricoGenerale().getDataControlloTempo())
			...
			.dataCreazione(dettaglio.getStoricoGenerale().getDataCreazione()).build();
	}
