...
public class AlimentazioneAScattiService {
	...
	public List<List<StoricoTotaleSingoloDto>> calcolaAlimentazioneList(Map<Long, Integer> idBagnoScattiParzialiMap) {
	...
	public List<RispostaScattiGenerale> calcolaRispostaList(Map<Long, Integer> idBagnoScattiParzialiMap) {
		List<RispostaScattiGenerale> risposta = new ArrayList<>();
		for (Map.Entry<Long, Integer> entry : idBagnoScattiParzialiMap.entrySet()) {
			if (entry.getValue() == null) {
				continue;
			}
			risposta.add(creaNuovaRisposta(entry.getKey(), entry.getValue()));
		}
		return risposta;
	}

	public RispostaScattiGenerale rispostaBuilder(StoricoGenerale storicoGenerale,
		String rispostaCalcoloFront,
		Integer restoscattiPrecedenti) {
		List<RispostaScattiDettaglio> dettaglioList = new ArrayList<>();
		RispostaScattiGenerale risposta = RispostaScattiGenerale
			.builder()
		...
		if (storicoDettaglioList == null || storicoDettaglioList.isEmpty()) {
			...
			for (StoricoDettaglio dettaglio : storicoDettaglioList) {
				...
				Double quantita = convertiQuantitaGenerico(dettaglio.getQuantita(),
					dettaglio.getUnitaDiMisura(),
					unita);
				RispostaScattiDettaglio rsd = RispostaScattiDettaglio
					.builder()
	...
	public RispostaScattiGenerale creaNuovaRisposta(Long idBagno, int scattiLetti) {
		calcolaAlimentazioneControlliApprovati(idBagno);
		Integer restoScattiPrecedenti =
			(storicoGeneraleRepository.ultimoStoricoGeneraleScatti(idBagno)).getRestoScattiBagno();
		...
		if (storicoGeneraleCreato.getMoltiplicatoreAlimentazione() == 0) {
			...
			return rispostaBuilder(storicoGeneraleCreato, messaggioFront, restoScattiPrecedenti);
		}
	...
	public List<StoricoTotaleSingoloDto> creaNuovaAlimentazione(Long idBagno, int scattiLetti) {
		...
		if (storicoGeneraleCreato.getMoltiplicatoreAlimentazione() == 0) {
			risposta.setRispostaCalcoloFront("gli scatti sono inferiori al 90% dell'alimentazione," +
				"le aggiunte non verranno eseguite ma messe in conto per la prossima chiamata.");
		...
		for (StoricoDettaglio storicoDettaglio : storicoDettaglioList) {
			...
			UnitaDiMisura unitaDiMisura = convertiUnitaMisuraPerDto(storicoDettaglio.getQuantita(),
				storicoDettaglio.getUnitaDiMisura().isSonoVolume());
			Double quantita = convertiQuantitaGenerico(storicoDettaglio.getQuantita().doubleValue(),
				storicoDettaglio.getUnitaDiMisura(),
				unitaDiMisura);
	...
	private StoricoTotaleSingoloDto buildaStoricoTotaleSingoloDtoSoloGenerale(StoricoGenerale generale) {
		return StoricoTotaleSingoloDto
			.builder()
			.idStoricoGenerale(generale.getIdStorico())
			.idBagno(generale.getBagno().getIdBagno())
			.nomeBagno(generale.getBagno().getNome())
			...
			.restoScattiPrecedenti(storicoGeneraleRepository
				.ultimoStoricoGeneraleScatti(generale.getBagno().getIdBagno())
				.getRestoScattiBagno())
	...
	private Integer trovaPrimoValoreVolumetrico(Alimentazione alimentazione) {
		...
		return alimentazione
			...
			.stream()
			.filter(dettaglioAlimentazione -> dettaglioAlimentazione
				.getUnitaDiMisura()
				.isSonoVolume())
			.findFirst()
	...
	private void creaStoricoDettaglioApprossimato(Alimentazione alimentazione,
		RisultatoOperazioniAScatti risultatoOperazioniAScatti,
	...
	protected Long creaStorici(Long idBagno, int scattiLetti) {
		...
		StoricoGenerale ultimoStoricoGenerale =
		...
		RisultatoOperazioniAScatti risultatoOperazioniAScatti =
			MetodiArrotondamenti.operazioniAScatti(scattiLetti,
		...
		StoricoGenerale storicoGenerale = StoricoGenerale
			.builder()
			.bagno(bagno)
			.scattiTotaliBagno(scattiTotali)
			.scattiInseriti(scattiLetti)
			.alimentazione(alimentazione)
			.tipologiaAggiunta(TipologiaAggiunta.SCATTI)
			.moltiplicatoreAlimentazione(risultatoOperazioniAScatti.getMoltiplicatoreAlimentazione())
			.build();

		if (risultatoOperazioniAScatti.getMoltiplicatoreAlimentazione() == 0) {
			storicoGenerale.setRestoScattiBagno(ultimoStoricoGenerale.getRestoScattiBagno() +
				scattiLetti);
		...
		creaStoricoDettaglioApprossimato(alimentazione,
	...
	private void calcolaAlimentazioneControlliApprovati(Long idBagno) {
		...
		if (bagno
			...
			.stream()
			.noneMatch(alimentazione -> alimentazione.getTipologiaAggiunta() ==
				TipologiaAggiunta.SCATTI)) {
	...
	private Alimentazione trovaAlimentazioneScatti(Bagno bagno) {
		return bagno
			...
			.findFirst()
			.orElseThrow(() -> new RuntimeException("Alimentazione a scatti non trovata per bagno " +
				bagno.getIdBagno() +
				" nome: " +
				bagno.getNome()));
	}
