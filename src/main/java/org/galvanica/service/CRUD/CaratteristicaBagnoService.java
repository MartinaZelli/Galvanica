...
@Service
public class CaratteristicaBagnoService
	implements ICRUDService<CaratteristicaBagnoDto, CaratteristicaBagno> {
	...
	public CaratteristicaBagnoDto inserisci(CaratteristicaBagnoDto elemento) {
		if (elemento.getIdBagno() == null) {
			throw new RuntimeException("Il Bagno deve essere valorizzato");
		}
		if (elemento.getNome() == null) {
			throw new RuntimeException("Il nome deve essere valorizzato");
		}
		if (elemento.getIdCaratteristica() != null) {
			throw new RuntimeException("L'id è autoincrementale, non inizializzare");
		}
		...
		CaratteristicaBagno
			caratteristicaBagno =
			CaratteristicaBagno.builder().nome(elemento.getNome()).bagno(bagnoTrovato.get())
				.descrizione(elemento.getDescrizione()).build();
	...
	public CaratteristicaBagnoDto aggiorna(CaratteristicaBagnoDto elemento) {
		...
		Optional<CaratteristicaBagno>
			caratteristicaOpt =
			repository.findById(elemento.getIdCaratteristica());
		if (caratteristicaOpt.isEmpty()) {
			throw new RuntimeException("mettere un id corretto");
		}
		if (elemento.getIdBagno() == null) {
			throw new RuntimeException("il bagno non può essere null");
		}
		Optional<Bagno> bagnoTrovato = repositoryBagno.findById(elemento.getIdBagno());
		if (bagnoTrovato.isEmpty()) {
			throw new RuntimeException("L'id del bagno non esiste. correggere.");
	...
	public Optional<CaratteristicaBagnoDto> ricercaId(long id) {
		Optional<CaratteristicaBagno> caratteristicaBagnoTrovato = repository.findById(id);
		return caratteristicaBagnoTrovato.map(this::fromModelToDto);
	}

	@Override
	public CaratteristicaBagnoDto fromModelToDto(CaratteristicaBagno oggettoDaTrasformare) {
	...
	public List<CaratteristicaBagnoDto> findAllCaratteristicaBagno() {
		return StreamSupport.stream(caratteristicaBagnoRepository.findAll().spliterator(), false)
