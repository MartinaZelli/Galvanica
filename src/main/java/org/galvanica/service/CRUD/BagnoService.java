...
public class BagnoService implements ICRUDService<BagnoDto, Bagno> {
	...
	public BagnoService(BagnoRepository repository, StoricoGeneraleRepository storicoGeneraleRepository) {
	...
	public BagnoDto inserisci(BagnoDto elemento) {
		...
		storicoGeneraleRepository.save(StoricoGenerale
			...
			.dataCreazione(LocalDateTime.now())
			.dataEsecuzione(LocalDateTime.now())
			.dataControlloTempo(LocalDate.now())
			.eseguitoGenerale(true)
	...
	public List<BagnoDto> findAllBagnoIfAlimentazioneScattiNotNull() {
		return repository
			.findAllBagnoIfAlimentazioneScattiNotNull()
			.stream()
			.map(this::fromModelToDto)
			.collect(Collectors.toList());
	}
