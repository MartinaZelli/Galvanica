...
public class BagnoService implements ICRUDService<BagnoDto, Bagno> {
	...
	public BagnoDto inserisci(BagnoDto elemento) {
		if (elemento.getNome() == null) {
			throw new RuntimeException("Il nome deve essere valorizzato");
		}
		...
		Bagno
			bagno =
			Bagno.builder().nome(elemento.getNome()).litri(elemento.getLitri())
				.dataInizio(LocalDate.now()).dataFine(null).build();
		bagno = repository.save(bagno);
		storicoGeneraleRepository.save(
			StoricoGenerale.builder().bagno(bagno).scattiTotaliBagno(elemento.getScattiTotali())
				.restoScattiBagno(elemento.getRestoScatti()).dataCreazione(LocalDateTime.now())
				.dataEsecuzione(LocalDateTime.now()).annullatoGenerale(true)
				.tipologiaAggiunta(TipologiaAggiunta.SCATTI).note("valori bagno iniziali").build());
	...
	public BagnoDto aggiorna(BagnoDto elemento) {
		...
		if (bagnoTrovato.isEmpty()) {
			throw new RuntimeException("metti un id corretto che questo non c'è, bischero.");
		}
		...
		storicoGeneraleRepository.save(
			StoricoGenerale.builder().bagno(bagno).scattiTotaliBagno(elemento.getScattiTotali())
				.restoScattiBagno(elemento.getRestoScatti()).dataCreazione(LocalDateTime.now())
				.dataEsecuzione(LocalDateTime.now()).eseguitoGenerale(true)
				.tipologiaAggiunta(TipologiaAggiunta.SCATTI).note("modifica manuale").build());
		return fromModelToDto(bagno);
	...
	public BagnoDto fromModelToDto(Bagno oggettoDaTrasformare) {
		return BagnoDto.builder().idBagno(oggettoDaTrasformare.getIdBagno())
			.nome(oggettoDaTrasformare.getNome()).litri(oggettoDaTrasformare.getLitri()).build();
	}
	...
	public List<StoricoGenerale> storicoGeneraleListByBagno(Long idBagno, Long limite) {
	...
	public boolean controllaSeBagnoHaAlimentazioneAScatti(Long idBagno) {
		...
		if (bagnoOptional.get().getAlimentazioneList() != null) {
			return bagnoOptional.get().getAlimentazioneList().stream()
				.anyMatch(alimentazione -> alimentazione.getScatti() != null);
		}
		return false;
	}

	// TODO: mettere Optional BagnoDto oppure senza Optional? decidere quale è il
	// migliore.
	/*
	 * @Override
