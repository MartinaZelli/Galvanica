...
public class MagazzinoService implements ICRUDService<MagazzinoDto, Magazzino> {
	...
	public MagazzinoDto inserisci(MagazzinoDto elemento) {
		...
		Magazzino magazzino = Magazzino.builder().descrizione(elemento.getDescrizione()).build();
	...
	public Optional<MagazzinoDto> ricercaId(long id) {
		...
		return magazzinoTrovato.map(this::fromModelToDto);
	}

	@Override
	public MagazzinoDto fromModelToDto(Magazzino oggettoDaTrasformare) {
		return MagazzinoDto.builder().idMagazzino(oggettoDaTrasformare.getIdMagazzino())
			.descrizione(oggettoDaTrasformare.getDescrizione()).build();
	}

	public List<MagazzinoDto> findAllMagazzino() {
		return StreamSupport.stream(magazzinoRepository.findAll().spliterator(), false)
