...
public class RelazioneBagnoProdottoController {
	...
	public RelazioneBagnoProdottoController(RelazioneBagnoProdottoService relazioneService,
		BagnoService bagnoService, ProdottoService prodottoService) {
	...
	public String inserisciRelazione(
		@RequestBody RelazioneBagnoProdottoDto relazioneBagnoProdottoDto, Model model) {

		return relazioneVistaAzioni(
			relazioneService.inserisci(relazioneBagnoProdottoDto).getIdBagno(), model);
	...
	public String relazioneVistaAzioni(@PathVariable Long id, Model model) {
		model.addAttribute("bagno", bagnoService.ricercaId(id).orElseThrow());
		model.addAttribute("relazioneList", relazioneService.findAllRelazionePerBagno(id));
	...
	public String relazioneAzioni(@PathVariable Long id, Model model) {
		model.addAttribute("prodottoList", prodottoService.findAllProdotto());
		model.addAttribute("relazione", relazioneService.ricercaId(id).orElseThrow());
		return "relazioneBagnoProdotto/relazioneAzioni";
	}

	@PutMapping("{id}")
	public String aggiornaRelazione(
		@RequestBody RelazioneBagnoProdottoDto relazioneBagnoProdottoDto, @PathVariable Long id,
		Model model) {
		relazioneBagnoProdottoDto.setIdRelazione(id);
		return relazioneVistaAzioni(
			relazioneService.aggiorna(relazioneBagnoProdottoDto).getIdBagno(), model);
	}
	...
	public String prodottoList(@PathVariable Long id, Model model) {
		model.addAttribute("prodottoList", prodottoService.ricercaProdottiByBagno(id));
